...
export default function RiwayatClient({ riwayatData, user }: RiwayatClientProps) {
    ...
    const filterByDateRange = (items: Riwayat[]) => {
    ...
    const handleExportExcel = () => {
        ...
        if (filteredData.length === 0) {
            setNotif({ show: true, success: false, message: 'Tidak ada riwayat peminjaman dalam range tanggal tersebut!' })
            return
        }
        ...
        const tableHtml = `
            ...
            <head>
                ...
                <xml>
                    <x:ExcelWorkbook>
                        <x:ExcelWorksheets>
                            <x:ExcelWorksheet>
                                <x:Name>Laporan Riwayat Peminjaman</x:Name>
                                <x:WorksheetOptions>
                                    <x:DisplayGridlines/>
                                </x:WorksheetOptions>
                            </x:ExcelWorksheet>
            ...
            <body>
                <h2 style="text-align: center;">Laporan Riwayat Peminjaman Buku</h2>
        ...
        link.href = url
        link.setAttribute('download', `Laporan_Riwayat_Peminjaman_${startDate}_to_${endDate}.xls`)
    ...
    const handleExportPdf = () => {
        ...
        const title = `Laporan Riwayat Peminjaman (${formatDate(startDate)} - ${formatDate(endDate)})`
                ...
                <tr>
                    ...
                    <td>
                        <span style="padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: bold; 
                            background-color: ${item.status === 'terlambat' ? '#fee2e2' : '#dcfce7'}; 
                            color: ${item.status === 'terlambat' ? '#b91c1c' : '#15803d'};">
                            ${item.status}
                        </span>
                    </td>
        ...
        printWindow.document.write(`
            ...
            <body>
                <div class="header">
                    <h1>Laporan Riwayat Peminjaman Buku</h1>
        ...
        <div className="px-4 py-6 space-y-5">
            ...
            <div className="grid grid-cols-3 gap-2">
                ...
                <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-3 text-white">
                    ...
                    <p className="text-xl font-bold">
                        <AnimatedCounter value={stats.dikembalikan} className="text-white font-bold text-xl" delay={0.1} />
                    </p>
